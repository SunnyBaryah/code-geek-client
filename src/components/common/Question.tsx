...
export default function Question(props:{question:QuestionData}){
    ...
    return (<div className=" text-white border-t-2 border-gray-900 px-4 py-3 max-h-[580px] overflow-auto">
        <h1 className='font-semibold text-2xl text-gray-200 mb-3'>{`${props.question.id}. ${props.question.title} `}</h1>
        <p className={`${getDifficultyClass(props.question.difficulty)} bg-gray-700 text-xs mb-5 max-w-fit border border-gray-400 px-1.5 py-0.5 rounded-xl`}>
        ...
        {props.question.examples.map((example, index)=>{
          return (<div key={index} className="text-gray-300 mb-4">
        ...
        {props.question.constraints.map((constraint, index)=>{
          return (<div key={index} className="text-gray-300 mb-2 ml-2">
            <p>{`• ${constraint.item}`}</p>
          </div>);
        })}
